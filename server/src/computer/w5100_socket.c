...
void
nic_w5100_socket_init( nic_w5100_socket_t *socket, int which, struct network_device_t* device )
{
  socket->id = which;
...
{
  socket->ir = 0;
  socket->type = W5100_SOCKET_TYPE_INET;
...
w5100_write_socket_mr( nic_w5100_socket_t *socket, uint8_t b )
{
  ...
  switch( mode ) {
    ...
    case W5100_SOCKET_MODE_TCP:
      ...
      if( flags != 0x20 ) {
        nic_w5100_error( "w5100: unsupported flags 0x%02x set for TCP mode on socket %d\n", b & 0xf0, socket->id );
        flags = 0x20;
      }
      break;
    case W5100_SOCKET_MODE_UDP:
      ...
      if( flags != 0x00 ) {
        nic_w5100_error( "w5100: unsupported flags 0x%02x set for UDP mode on socket %d\n", b & 0xf0, socket->id );
        flags = 0;
      }
      break;
    ...
    default:
      nic_w5100_error( "w5100: unsupported mode 0x%02x set on socket %d\n", b, socket->id );
...
static uint8_t
w5100_socket_check_allocate( nic_w5100_socket_t *socket_obj, w5100_socket_type socket_type )
{
    if (socket_obj->fd != compat_socket_invalid)
        return 0;

    if( socket_obj->state == W5100_SOCKET_STATE_INIT || socket_obj->state == W5100_SOCKET_STATE_UDP ) {
        ...
        const char *description = socket_type == W5100_SOCKET_TYPE_UNIX ? "UNIX" : (tcp ? "TCP" : "UDP");
        int domain = socket_type == W5100_SOCKET_TYPE_UNIX ? AF_UNIX : AF_INET;
        int type = socket_type == W5100_SOCKET_TYPE_UNIX ? SOCK_STREAM : (tcp ? SOCK_STREAM : SOCK_DGRAM);
        int protocol = socket_type == W5100_SOCKET_TYPE_UNIX ?
            0 : (tcp ? IPPROTO_TCP : IPPROTO_UDP);

        socket_obj->fd = socket( domain, type, protocol );
        ...
        nic_w5100_debug( "w5100: actually opened %s fd %d for socket %d\n", description, socket_obj->fd, socket_obj->id );
    }
...
w5100_socket_open( nic_w5100_socket_t *socket_obj )
{
  if( ( socket_obj->mode == W5100_SOCKET_MODE_UDP ||
    ...
    int final_state = tcp ? W5100_SOCKET_STATE_INIT : W5100_SOCKET_STATE_UDP;
    w5100_socket_clean( socket_obj );
    socket_obj->fd = compat_socket_invalid;
    socket_obj->state = final_state;
    nic_w5100_debug( "w5100: \"opened\" %s for socket %d\n", description, socket_obj->id );
  }
}

void
w5100_socket_unix_gen_path(uint16_t device_id, uint16_t namespace_id, uint16_t port, struct sockaddr_un* un)
{
    un->sun_family = AF_UNIX;
    snprintf(un->sun_path, sizeof(un->sun_path) - 1, "/tmp/unbound%d_dev%d_port%d", namespace_id, device_id, port);
}

static int
w5100_socket_bind_port( nic_w5100_t *self, nic_w5100_socket_t *socket, uint8_t as_server )
{
  if (as_server)
  {
      if (w5100_socket_check_allocate(socket, W5100_SOCKET_TYPE_UNIX))
      {
          return -1;
      }

      struct sockaddr_un un;
      w5100_socket_unix_gen_path(socket->device->device_id, socket->device->namespace_id, ntohs(socket->port_uint16), &un);

      nic_w5100_debug( "w5100: attempting to bind socket %d to path %s\n", socket->id, un.sun_path );

      unlink(un.sun_path);

      if (bind(socket->fd, (struct sockaddr*)&un, sizeof(un)) == -1) {
          nic_w5100_error(
      ...
      nic_w5100_debug( "w5100: successfully bound socket %d to path %s\n", socket->id, un.sun_path );
  }
  else
  {
      ...
      if (w5100_socket_check_allocate(socket, W5100_SOCKET_TYPE_INET))
      {
          return -1;
      }

      // bind to 0.0.0.0/0 (only happens for udp)
...
w5100_socket_listen( nic_w5100_t *self, nic_w5100_socket_t *socket )
{
  ...
  if( socket->state == W5100_SOCKET_STATE_INIT ) {
    ...
    if( !socket->socket_bound )
      if( w5100_socket_bind_port( self, socket, 1 ) )
        return;
...
{
    uint32_t target_ip = ntohl(socket->dip_uint32);
    w5100_socket_type typ = network_is_ip_local(target_ip) ? W5100_SOCKET_TYPE_UNIX : W5100_SOCKET_TYPE_INET;
    if (w5100_socket_check_allocate(socket, typ))
    {
        return;
    }
  ...
  if( socket->state == W5100_SOCKET_STATE_INIT ) {
    ...
    if (network_is_ip_local(target_ip))
    {
        // connect to a locally bound port instead
        struct sockaddr_un un;

        w5100_socket_unix_gen_path(
            network_get_device_id_form_local_ip(target_ip),
            socket->device->namespace_id,
            ntohs(socket->dport_uint16), &un);

        if( connect( socket->fd, (struct sockaddr*)&un, sizeof(un)) == -1 ) {
            nic_w5100_error(
                "w5100: failed to connect to unix socket %d to path %s; errno %d: %s\n",
                socket->id, un.sun_path,
...
w5100_socket_send( nic_w5100_t *self, nic_w5100_socket_t *socket )
{
  ...
  if( socket->state == W5100_SOCKET_STATE_UDP ) {
    ...
    if( !socket->socket_bound )
      if( w5100_socket_bind_port( self, socket, socket->listening ) )
        return;
...
w5100_write_socket_cr( nic_w5100_t *self, nic_w5100_socket_t *socket, uint8_t b )
{
  ...
  switch( b ) {
    ...
    case W5100_SOCKET_COMMAND_RECV:
      ...
      break;
    default:
      nic_w5100_error( "w5100: unknown command 0x%02x sent to socket %d\n", b, socket->id );
      break;
  }
}
...
static void
w5100_socket_place_data_on_read( nic_w5100_socket_t *socket, uint8_t* buffer, uint16_t bytes_read,
                                 int udp, struct sockaddr_in* sa)
{
...
w5100_socket_process_read( nic_w5100_socket_t *socket )
{
  ...
  nic_w5100_verbose( "w5100: reading from socket %d\n", socket->id );
  ...
  nic_w5100_verbose( "w5100: read 0x%03x bytes from %s socket %d\n", (int)bytes_read, description, socket->id );

  if( bytes_read > 0 || (udp && bytes_read == 0) ) {
      w5100_socket_place_data_on_read(socket, buffer, bytes_read, udp, &sa);
  }
...
static void
w5100_socket_process_udp_write( nic_w5100_t *self, nic_w5100_socket_t *socket )
{
  ssize_t bytes_sent;
  ...
  uint8_t buffer[0x800];
    ...
    nic_w5100_verbose( "w5100: writing to UDP socket %d to %s port %d\n", socket->id,
                       inet_ntoa(sa.sin_addr), htons(socket->dport_uint16) );

    uint8_t do_send = 1;

    if (htons(socket->dport_uint16) == 53)
    {
        char domain_name[128];
        uint16_t request_id;
        if (dns_parse((uint8_t*)data, length, domain_name, &request_id) == 0)
        {
            size_t l = strlen(domain_name);
            if ((l > 4) && (strcmp(domain_name + l - 4, ".zx.") == 0))
            {
                char subdomain[128] = {};
                sprintf(subdomain, "%.*s", (int)l - 4, domain_name);
                struct  in_addr d;
                struct network_device_t* other = network_lookup_device(socket->device, subdomain);

                if (other)
                {
                    d.s_addr = ntohl(network_get_local_ip_from_device(other));
                    nic_w5100_debug( "w5100: resolved %s to %s locally\n", domain_name, inet_ntoa(d));
                }
                else
                {
                    d.s_addr = 0;
                    nic_w5100_debug( "w5100: could not resolved %s\n", domain_name);
                }

                uint32_t output_size;
                if (dns_encode(domain_name, request_id, d.s_addr, buffer + 8, &output_size) == 0)
                {
                    w5100_socket_place_data_on_read(socket, buffer, output_size + 8, 1, &sa);
                    do_send = 0;
                }
            }
        }
    ...
    if (do_send)
    {
        uint32_t target_addr = ntohl(socket->dip_uint32);
        if (network_is_ip_local(target_addr))
        {
            // not possible
            return;
        }
  ...
  if (do_send)
  {
      ...
      nic_w5100_verbose( "w5100: sent 0x%03x bytes of 0x%03x to UDP socket %d\n",
...
w5100_socket_process_tcp_write( nic_w5100_socket_t *socket )
{
  ...
  nic_w5100_verbose( "w5100: writing to TCP socket %d\n", socket->id );
  ...
  nic_w5100_verbose( "w5100: sent 0x%03x bytes of 0x%03x to TCP socket %d\n",
                   (int)bytes_sent, length, socket->id );
...
void
nic_w5100_socket_process_io( struct nic_w5100_t *self, nic_w5100_socket_t *socket, fd_set readfds,
  fd_set writefds )
{
  w5100_socket_acquire_lock( socket );
  ...
  if( socket->fd != compat_socket_invalid && socket->ok_for_io ) {
    ...
    if( FD_ISSET( socket->fd, &writefds ) ) {
      if( socket->state == W5100_SOCKET_STATE_UDP ) {
        w5100_socket_process_udp_write( self, socket );
      }
      else if( socket->state == W5100_SOCKET_STATE_ESTABLISHED ) {
