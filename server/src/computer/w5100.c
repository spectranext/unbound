...
w5100_io_thread( void *arg )
{
  ...
  while( !self->stop_io_thread ) {
    ...
    nic_w5100_verbose( "w5100: io thread select\n" );

    active = select( max_fd + 1, &readfds, &writefds, NULL, NULL );

    nic_w5100_verbose( "w5100: io thread wake; %d active\n", active );

    if( active != -1 ) {
      if( FD_ISSET( selfpipe_socket, &readfds ) ) {
        nic_w5100_verbose( "w5100: discarding selfpipe data\n" );
...
void
nic_w5100_init( nic_w5100_t* self, void* compat_user )
{
  int error;
  int i;

  nic_w5100_debug( "w5100: init\n" );

  compat_socket_selfpipe_init(&self->selfpipe);

  for( i = 0; i < 4; i++ )
    nic_w5100_socket_init( &self->socket[i], i, compat_user );
  ...
  if( error ) {
    nic_w5100_error(  "w5100: error %d creating thread", error );
  }
}

void
nic_w5100_destroy( nic_w5100_t *self )
{
    int i;
    ...
    compat_socket_selfpipe_destroy( &self->selfpipe );

    nic_w5100_debug( "w5100: destroy\n" );
}

uint8_t
nic_w5100_read( nic_w5100_t *self, uint16_t reg )
{
  uint8_t b;
  ...
  if( reg < 0x030 ) {
    ...
    b = 0xff;
    nic_w5100_error( "w5100: reading 0x%02x from unsupported register 0x%03x\n", b, reg );
  }
...
{
  nic_w5100_verbose( "w5100: writing 0x%02x to MR\n", b );

  if( b & 0x80 )
    nic_w5100_reset( self );

  if( b & 0x7f )
    nic_w5100_error( "w5100: unsupported value 0x%02x written to MR\n", b );
}

static void
w5100_write_imr( nic_w5100_t *self, uint8_t b )
{
  nic_w5100_verbose( "w5100: writing 0x%02x to IMR\n", b );

  if( b != 0xef )
    nic_w5100_error( "w5100: unsupported value 0x%02x written to IMR\n", b );
}
...
w5100_write__msr( nic_w5100_t *self, uint16_t reg, uint8_t b )
{
  ...
  nic_w5100_verbose( "w5100: writing 0x%02x to %s\n", b, regname );

  if( b != 0x55 )
    nic_w5100_error( "w5100: unsupported value 0x%02x written to %s\n",
                     b, regname );
}
  ...
  if( reg < 0x030 ) {
    switch( reg ) {
      case W5100_MR:
        ...
        break;
      case W5100_GWR0: case W5100_GWR1: case W5100_GWR2: case W5100_GWR3:
        nic_w5100_verbose( "w5100: writing 0x%02x to GWR%d\n", b, reg - W5100_GWR0 );
        self->gw[reg - W5100_GWR0] = b;
        break;
      case W5100_SUBR0: case W5100_SUBR1: case W5100_SUBR2: case W5100_SUBR3:
        nic_w5100_verbose( "w5100: writing 0x%02x to SUBR%d\n", b, reg - W5100_SUBR0 );
        self->sub[reg - W5100_SUBR0] = b;
        break;
      case W5100_SHAR0: case W5100_SHAR1: case W5100_SHAR2:
      case W5100_SHAR3: case W5100_SHAR4: case W5100_SHAR5:
        nic_w5100_verbose( "w5100: writing 0x%02x to SHAR%d\n", b, reg - W5100_SHAR0 );
        self->sha[reg - W5100_SHAR0] = b;
        break;
      case W5100_SIPR0: case W5100_SIPR1: case W5100_SIPR2: case W5100_SIPR3:
        nic_w5100_verbose( "w5100: writing 0x%02x to SIPR%d\n", b, reg - W5100_SIPR0 );
